...
# ─── 2) Paths & settings ──────────────────────────────────────────────────────
# Single image
dataset_image  = "dataset/violet_evergarden_15.png"
# SR epoch settings
...
# ─── 4) Define your Real-ESRGAN upsamplers ────────────────────────────────────
...
model_anime = RRDBNet(num_in_ch=3, num_out_ch=3, num_feat=64, num_block=6, num_grow_ch=32, scale=scale_factor)
...
def process_image(img: Image.Image, crop_size=512, low_res_size=128):
    ...
    to_t = T.ToTensor()
    hr_t = to_t(crop).unsqueeze(0).to(device) * 2.0 - 1.0
    lr_t = to_t(lr).unsqueeze(0).to(device) * 2.0 - 1.0
    return lr, hr_t, lr_t

# ─── 6) Build matplotlib grid ─────────────────────────────────────────────────
# One row, four columns: SR Epoch | Fine-tuned | Anime | Original
fig = plt.figure(figsize=(16, 4))  # 4 cols × ~4" each
...
img = Image.open(dataset_image).convert('RGB')
...
# 7a) SR epoch model
with torch.no_grad():
    sr_ep_t = sr_model(lr_t)
mse_ep = F.mse_loss(sr_ep_t, hr_t).item()
psnr_ep = -10 * np.log10(mse_ep)
l1_ep   = F.l1_loss(sr_ep_t, hr_t).item()
sr_ep   = sr_ep_t.cpu().squeeze(0).permute(1,2,0).numpy() * 0.5 + 0.5
sr_ep   = np.clip(sr_ep, 0, 1)

# 7b) Fine-tuned Real-ESRGAN
...
psnr_ft = -10 * np.log10(F.mse_loss(sr_ft_t, hr_t).item())
l1_ft   = F.l1_loss(sr_ft_t, hr_t).item()

# 7c) Anime Real-ESRGAN
...
psnr_an = -10 * np.log10(F.mse_loss(sr_an_t, hr_t).item())
...
ax.imshow(sr_ep)
ax.axis('off')
ax.text(0.02, 0.02, f"{psnr_ep:.1f} dB\nL1: {l1_ep:.3f}",
...
plt.savefig(f"realesrgan_one_row_with_sr_epoch_{epoch_to_compare}.png", dpi=300, bbox_inches="tight")
plt.show()
